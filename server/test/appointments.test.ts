...
describe('POST /api/v1/appointments', () => {
  it('books an online slot with correct fee_paise, APT-xxxxxx id and echoed razorpay order', async () => {
    ...
    // The booking response carries no video link. Both sides join through the
    // *-token endpoints, which mint a signed JaaS session; a bare room URL is not
    // a working join link, so nothing stores or returns one.
    expect(res.body.appointment.videoCallLink).toBeUndefined();
