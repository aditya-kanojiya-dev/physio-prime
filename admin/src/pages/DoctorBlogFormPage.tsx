...
export function DoctorBlogFormPage() {
  ...
  const { data: existing, isLoading: loadingPost } = useQuery({
    queryKey: ['doctor/blog/post', id],
    queryFn: async () => api.get<{ post: BlogPost }>(`/doctor/blog/posts/${id}`),
    enabled: !isNew && !!id,
  })

  const { data: categoriesData } = useQuery({
    queryKey: ['doctor/blog/categories'],
    queryFn: async () => api.get<{ categories: BlogCategory[] }>('/doctor/blog/categories'),
  })

  const { data: tagsData } = useQuery({
    queryKey: ['doctor/blog/tags'],
    queryFn: async () => api.get<{ tags: BlogTag[] }>('/doctor/blog/tags'),
  })
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: Record<string, unknown>) => {
      if (isNew) return api.post<{ post: BlogPost }>('/doctor/blog/posts', payload)
      return api.patch<{ post: BlogPost }>(`/doctor/blog/posts/${id}`, payload)
    },
    onSuccess: (res) => {
      queryClient.invalidateQueries({ queryKey: ['doctor/blog/posts'] })
      navigate(`/blogs/${res.post.id}`)
    },
  })

  const deleteMutation = useMutation({
    mutationFn: async () => api.delete(`/doctor/blog/posts/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['doctor/blog/posts'] })
      navigate('/blogs')
    },
      ...
      <div className="space-y-6 max-w-4xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/blogs')} className="p-2 rounded-xl text-slate-500 hover:bg-slate-100 transition-all">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-slate-500 mt-0.5">
                {isNew ? 'Create a new article.' : `Editing: ${existing?.post?.title || ''}`}
              </p>
            </div>
          </div>
