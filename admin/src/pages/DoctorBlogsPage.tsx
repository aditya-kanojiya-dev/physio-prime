...
export function DoctorBlogsPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['doctor/blog/posts', search, status, page],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), pageSize: '20' })
      if (search) params.set('q', search)
      if (status !== 'all') params.set('status', status)
      return api.get<BlogPostsResponse>(`/doctor/blog/posts?${params}`)
    },
  })

  const deleteMutation = useMutation({
    mutationFn: async (id: number) => api.delete(`/doctor/blog/posts/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['doctor/blog/posts'] }),
  })
    ...
    <AdminLayout portal="doctor">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">My Blog Posts</h1>
            <p className="text-xs font-semibold text-slate-500 mt-1">Write and manage your articles.</p>
          </div>
          <Link
            to="/blogs/new"
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ['doctor/blog/posts'] })}
