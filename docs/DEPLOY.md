...
| Express API | `api/` (`src/index.ts`) | One Vercel Node function via `api/index.ts` |
...
- **`api/index.ts`** is not type-checked by `tsc -b` (api `tsconfig.json` only covers `src/`); Vercel compiles it itself. Local API runs stay on `api/src/server.ts` via `npm run dev -w api`.
- The root `package.json` `build` script builds all three workspaces — used by CI/local; Vercel uses the per-project build command above so builds stay fast.
